...
(function(){
  ...
  function markerPreviewPlacement(anchor,stageRect,previewRect){var width=Math.max(120,previewRect.width||208),height=Math.max(48,previewRect.height||54),left=clamp(anchor.x-(width/2),8,Math.max(8,stageRect.width-width-8)),above=anchor.y>=height+10;return {left:Math.round(left-anchor.x),vertical:above?'above':'below'};}
  ...
  function positionMarkerPreview(marker,x,y,height){var preview=marker.querySelector('.vanchor-preview'),rect=overlay.getBoundingClientRect();if(!preview||!rect.width)return;var placement=markerPreviewPlacement({x:x,y:y,h:height||0},rect,preview.getBoundingClientRect());preview.style.left=placement.left+'px';preview.style.transform='translateY(.2rem)';if(placement.vertical==='above'){preview.style.top='auto';preview.style.bottom='calc(100% + .5rem)';}else{preview.style.bottom='auto';preview.style.top=((height||24)+8)+'px';}marker.setAttribute('data-preview-side',placement.vertical);}
