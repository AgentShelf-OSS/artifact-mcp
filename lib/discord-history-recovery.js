...
export async function recoverExactNotification({
  ...
  for (let page = 0; page < maxPages && scanned < maxMessages; page += 1) {
    ...
    for (const message of result.messages.slice(0, maxMessages - scanned)) {
      ...
      // Discord's channel-history contract is newest-to-oldest. A selected-webhook message with
      // redacted embeds could itself be the newest match, so fail closed before considering older
      // messages. Otherwise the first exact match is the deterministic recovery anchor.
      if (evaluation?.redacted) return { state: "unavailable" };
      if (evaluation?.id) {
        return {
          state: "recovered",
          messageId: evaluation.id,
          provenance: EXACT_MATCH_PROVENANCE
        };
      }
    }
