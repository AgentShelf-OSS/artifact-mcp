...
export function createArtifactStore({
  ...
  function publish({ clientId, org, ownerEmail, uploaderLabel, html, title, description, category }) {
    ...
    try {
      ...
      inserted = true;
      recordRevisionRow(getMetaStmt.get(id), clientId);
  ...
  function pruneHistory(id) {
    try {
      const current = getMetaStmt.get(id)?.revision ?? 0;
      ...
      if (name.startsWith(".") && (name.includes(".staging-") || name.includes(".trash-"))) {
        ...
        if (cleanTransient) {
          ...
          if (row && !files.existsSync(finalPath)) {
            ...
            if (outgoing) snapshotBody(row.id, outgoing, { moveBody: true });
            else safeRemove(files, finalPath);
            files.renameSync(transient, finalPath);
            recoveredPaths.push(name);
          } else {
  ...
  return {
    ...
    listOrgArtifacts: (org, { includeHidden = false, ownerEmail = null } = {}) => ownerEmail && !includeHidden
      ? listByOrgForOwner.all(org, ownerEmail)
      : (includeHidden ? listByOrgIncludingHidden : listByOrg).all(org),
    listOrgIds: (org, { includeHidden = false, ownerEmail = null } = {}) => ownerEmail && !includeHidden
      ? listIdsByOrgForOwner.all(org, ownerEmail).map((row) => row.id)
      : (includeHidden ? listIdsByOrgIncludingHidden : listIdsByOrg).all(org).map((row) => row.id),
