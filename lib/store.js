...
export function createArtifactStore({
  ...
  // Discord discussion state is also organization-owned. Keep the artifact's local feedback,
  // but detach every source-org delivery binding so the destination cannot inherit another
  // tenant's Discord connection, thread, message mapping, recovery state, or inbound policy.
  // Provider outbox rows are immutable delivery history and deliberately remain in place.
  const dropDiscussionStateOnMove = [
  ...
  ].map((table) => database.prepare(`DELETE FROM ${table} WHERE artifact_id = ?`));
  ...
  function moveArtifactToOrg(id, targetOrg, category) {
    ...
    database.transaction(() => {
      ...
      if (org !== meta.org) {
        for (const statement of dropDiscussionStateOnMove) statement.run(id);
      }
