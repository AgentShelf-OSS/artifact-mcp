...
const emailsForStmt = db.prepare("SELECT email, display_name FROM org_email_members WHERE org = ? ORDER BY email ASC");
...
const insertEmailMemberStmt = db.prepare("INSERT INTO org_email_members (email, org, display_name) VALUES (?, ?, ?) ON CONFLICT(email) DO UPDATE SET display_name = excluded.display_name");
const viewerNameStmt = db.prepare("SELECT display_name FROM org_email_members WHERE email = ?");
...
export function addEmailMember(org, email, displayName) {
  org = String(org || "").trim();
  const normalized = normEmail(email);
  if (!orgExists(org)) throw new Error(`Unknown organization "${org}".`);
  ...
  if (displayName !== undefined && (typeof displayName !== "string" || /[\u0000-\u001f\u007f-\u009f]/u.test(displayName))) {
    throw new Error("display_name must be a single line of at most 40 characters.");
  }
  const name = displayName === undefined ? "" : displayName.trim();
  if ([...name].length > 40) throw new Error("display_name must be a single line of at most 40 characters.");
  if (owner && owner.org !== org) throw new Error(`Email "${normalized}" is already mapped to "${owner.org}".`);
  if (owner && owner.org === org && displayName === undefined) throw new Error(`"${normalized}" is already on this org.`);
  insertEmailMemberStmt.run(normalized, org, name);
  return displayName === undefined
    ? { org, email: normalized }
    : { org, email: normalized, display_name: name };
}

export function viewerDisplayName(email) {
  const row = viewerNameStmt.get(normEmail(email));
