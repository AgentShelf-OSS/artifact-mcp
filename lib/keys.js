...
const keyStmt = db.prepare("SELECT client_id, org, label, role, owner_email FROM api_keys WHERE client_id = ?");
...
const updateStmt = db.prepare("UPDATE api_keys SET label = ?, role = ?, owner_email = ? WHERE client_id = ?");
...
// Credential identity, tenant, creation, and revocation history are immutable. Existing artifacts
// retain publish-time attribution snapshots, while these fields govern future use of the key.
const updateKeyTransaction = db.transaction((clientId, input) => {
  const key = keyStmt.get(String(clientId || ""));
  if (!key) return null;
  const label = String(input?.label || "").trim().slice(0, 60);
  const role = String(input?.role || "").trim();
  if (!KEY_ROLES.has(role)) throw new Error(INVALID_KEY_ROLE_MESSAGE);
  const ownerEmail = normalizedVerifiedOwner(input?.ownerEmail, key.org);
  updateStmt.run(label, role, ownerEmail, key.client_id);
  return { clientId: key.client_id, org: key.org, label, role, ownerEmail };
});

export function updateKey(clientId, input) {
  return updateKeyTransaction(clientId, input);
}
