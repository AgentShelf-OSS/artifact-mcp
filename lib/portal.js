...
export function renderGallery(viewer, sections, reactions = new Map(), sentiment = new Map(), viewCounts = new Map(), topViewed = new Map(), orgColors = {}, notificationState = {}) {
  ...
  const categoryOptions = categoryNames
    .map((category) => `<option value="${esc(category)}">${esc(category || "Uncategorized")} (${allItems.filter((artifact) => (artifact.category && artifact.category.trim() ? artifact.category.trim() : "") === category).length})</option>`)
    .join("");
  ...
  <main id="stage">
    ...
    <section class="collection-tools" aria-label="Artifact library controls">
      ...
      <div class="toolbar-actions">
        ...
        <label class="select-control"><span class="sr-only">Category</span><select id="category-filter" aria-label="Filter by category"><option value="all">All categories</option>${categoryOptions}</select></label>
