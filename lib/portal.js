...
<div id="shell-config" hidden data-artifact-id="${attrLiteral(meta.id)}" data-prev-id="${attrLiteral(nav.prevId || "")}" data-next-id="${attrLiteral(nav.nextId || "")}" data-bundle-raw-prefix="${attrLiteral(`/raw/${meta.id}/`)}" data-version-query="${attrLiteral(versionQuery)}" data-viewer-email="${attrLiteral(identity.viewerEmail)}" data-viewer-is-admin="${identity.isAdmin ? "1" : "0"}" data-state-enabled="${identity.viewerEmail ? "1" : "0"}" data-feedback="${esc(feedbackData)}" data-title="${attrLiteral(meta.title)}" data-favorite="${reaction.favorite ? "1" : "0"}" data-vote="${Number(reaction.vote || 0)}" data-is-bundle="${meta.is_bundle ? "1" : "0"}" data-revision="${Number(meta.revision || 1)}" data-bytes="${Number(meta.bytes || 0)}"></div><script>${SHELL_SCRIPT}</script></body></html>`;
