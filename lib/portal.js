...
import { deriveViewerId, deriveViewerName } from "./viewer-identity.js";
...
// Encode a value as a JS literal safe to embed inside an inline <script>. It escapes the
...
const ICONS = {
  ...
  comment: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4Z"></path><path d="M8 9h8M8 13h5"></path></svg>`,
...
function card(a, reaction = {}, aggregate = {}, showAggregate = false, views = null, orgNames = [], categories = [], orgAccent = null) {
  ...
  const categoryOptions = categories
    .filter((value) => value)
    .map((value) => `<option value="${esc(value)}"${value === category ? " selected" : ""}>${esc(value)}</option>`)
    .join("");
    ...
    <div class="label">
      ...
      <div class="actions">
        ...
        <div class="card-menu" id="${menuId}" data-ui="card-menu" hidden>
          <label>Category<select class="category-menu" data-action="category" data-can-create="${showVisibility ? 1 : 0}" aria-label="Change category for ${esc(a.title)}"><option value=""${category ? "" : " selected"}>Uncategorized</option>${categoryOptions}${showVisibility ? '<option value="__create_category__">+ Category</option>' : ""}</select></label>
...
export function renderGallery(viewer, sections, reactions = new Map(), sentiment = new Map(), viewCounts = new Map(), topViewed = new Map(), orgColors = {}, notificationState = {}, orgCategories = {}) {
  const reactionFor = (id) => reactions.get(id) || { favorite: 0, vote: 0 };
  const isFav = (id) => !!reactionFor(id).favorite;
  const total = sections.reduce((n, s) => n + s.items.length, 0);
  const organizationTotal = sections.length;
  ...
  const categoryIndex = Object.fromEntries(sections.map((section) => {
    const counts = new Map();
    section.items.forEach((artifact) => {
      const name = artifact.category && artifact.category.trim() ? artifact.category.trim() : "";
      counts.set(name, (counts.get(name) || 0) + 1);
    });
    const configured = orgCategories instanceof Map
      ? orgCategories.get(section.org)
      : orgCategories[section.org];
    const names = [...new Set([
      ...(Array.isArray(configured) ? configured.map((name) => String(name || "").trim()).filter(Boolean) : []),
      ...counts.keys(),
    ])].sort((left, right) => {
      if (!left) return 1;
      if (!right) return -1;
    ...
    return [section.org, names.map((name) => ({ name, count: counts.get(name) || 0 }))];
  }));
  const categoryCountMap = new Map();
  Object.values(categoryIndex).flat().forEach(({ name, count }) => {
    categoryCountMap.set(name, (categoryCountMap.get(name) || 0) + count);
  });
  const categoryNames = [...categoryCountMap.keys()]
    .sort((left, right) => {
      if (!left) return 1;
      if (!right) return -1;
      return left.toLowerCase().localeCompare(right.toLowerCase());
  ...
  const hiddenTotal = allItems.filter((artifact) => artifact.hidden).length;
  ...
  const organizationOptions = sections
    .map((section) => `<option value="${esc(section.org)}">${esc(section.org)} (${section.items.length})</option>`)
    .join("");
  const categoryOptions = categoryNames
    .map((category) => `<option value="${esc(category)}">${esc(category || "Uncategorized")} (${categoryCountMap.get(category) || 0})</option>`)
    .join("");
  const viewFilters = `<div class="quick-filters" id="view-filters" aria-label="Quick filters">
      ...
      ${viewer.isAdmin ? `<button class="filter-choice" data-filter-view="hidden" aria-pressed="false">Hidden <span>${hiddenTotal}</span></button>` : ""}
  </div>`;
  ...
  const body =
    total === 0
      ...
      : `<div class="artifact-grid" id="artifact-grid" data-layout="grid">
          ${allItems.map((artifact) => card(artifact, reactionFor(artifact.id), sentiment.get(artifact.id) || {}, viewer.isAdmin, viewCounts.get(artifact.id) || null, orgNames, (categoryIndex[artifact.org] || []).map(({ name }) => name), orgColors[artifact.org])).join("")}
        </div>`;
...
<div class="wrap">
  ...
  <main id="stage">
    <section class="collection-head" aria-labelledby="page-title">
      ...
      <div class="collection-stats" aria-label="${total} artifacts, ${organizationTotal} organizations, ${needsReviewTotal} need review, ${hiddenTotal} hidden">
        <span><strong>${total}</strong>artifacts</span>
        <span><strong>${organizationTotal}</strong>organizations</span>
        <span><strong>${needsReviewTotal}</strong>need review</span>
        <span><strong>${hiddenTotal}</strong>hidden</span>
      </div>
    </section>
    ...
    <section class="collection-tools" aria-label="Artifact library controls">
      ...
      <div class="toolbar-actions">
        ${viewFilters}
        <label class="select-control"><span class="sr-only">Organization</span><select id="org-filter" aria-label="Filter by organization"><option value="all">All organizations</option>${organizationOptions}</select></label>
        <label class="select-control"><span class="sr-only">Category</span><select id="category-filter" aria-label="Filter by category"><option value="all">All categories</option>${categoryOptions}${viewer.isAdmin ? '<option value="__create_category__">+ Category</option>' : ""}</select></label>
    ...
    <section class="collection-results" aria-label="Artifact results">
        <span id="org-category-data" hidden data-json="${esc(JSON.stringify(categoryIndex))}" data-can-create="${viewer.isAdmin ? 1 : 0}"></span>
...
<body><main class="nf"><div class="brand"><span class="mark" aria-hidden="true">${esc(APP_BRAND)}</span><span><strong>${esc(APP_NAME)}</strong><small>${esc(SITE_HOST)}</small></span></div><div class="code">404 · Missing folio</div><h1>This artifact isn’t in the index.</h1><p>${esc(msg)}</p><a class="home" href="/">← Back to ${esc(APP_NAME)}</a></main></body></html>`;
}
...
export function renderArtifactShell(meta, nav, reaction = { favorite: 0, vote: 0 }, feedback = [], analytics = {}, viewer = {}, orgAccent = null) {
  ...
  const identity = {
    ...
    viewerId: deriveViewerId(viewer.email),
    viewerName: viewer.displayName || deriveViewerName(viewer.email)
  };
...
<div id="shell-config" hidden data-artifact-id="${attrLiteral(meta.id)}" data-prev-id="${attrLiteral(nav.prevId || "")}" data-next-id="${attrLiteral(nav.nextId || "")}" data-bundle-raw-prefix="${attrLiteral(`/raw/${meta.id}/`)}" data-version-query="${attrLiteral(versionQuery)}" data-viewer-email="${attrLiteral(identity.viewerEmail)}" data-viewer-id="${attrLiteral(identity.viewerId || "")}" data-viewer-name="${attrLiteral(identity.viewerName)}" data-viewer-is-admin="${identity.isAdmin ? "1" : "0"}" data-state-enabled="${identity.viewerEmail ? "1" : "0"}" data-feedback="${esc(feedbackData)}" data-title="${attrLiteral(meta.title)}" data-favorite="${reaction.favorite ? "1" : "0"}" data-vote="${Number(reaction.vote || 0)}" data-is-bundle="${meta.is_bundle ? "1" : "0"}" data-revision="${Number(meta.revision || 1)}" data-bytes="${Number(meta.bytes || 0)}"></div><script>${SHELL_SCRIPT}</script></body></html>`;
