...
export function notFoundPage(message) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark"><link rel="icon" href="${PORTAL_FAVICON}"><title>Not found &middot; ${esc(APP_NAME)}</title><script>${THEME_BOOT}</script><style>${NOT_FOUND_CSS}</style></head><body><main class="nf"><div class="code">404 · Missing folio</div><h1>This artifact isn’t in the index.</h1><p>${esc(msg)}</p><a class="home" href="/">← Back to ${esc(APP_NAME)}</a></main></body></html>`;
}
