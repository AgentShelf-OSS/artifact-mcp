...
import { injectAnchorBridge, injectReaderBridge, isHtmlContentType, rawArtifactHeaders, stripScripts } from "./artifact-http.js";
...
import { STATE_JSON_LIMIT, STATE_MAX_KEY_LENGTH, STATE_PER_WINDOW, STATE_RATE_WINDOW_MS } from "./config.js";
import { SPEECH_MAX_CHARS, createSpeechService, speechEnabledFor } from "./speech.js";

const SOUNDTOUCH_PROCESSOR = readFileSync(new URL("../assets/vendor/soundtouch-processor.js", import.meta.url), "utf8");
const SOUNDTOUCH_SOURCE_MAP = readFileSync(new URL("../assets/vendor/soundtouch-processor.js.map", import.meta.url), "utf8");
const SOUNDTOUCH_LICENSE = readFileSync(new URL("../assets/vendor/soundtouch-LICENSE.txt", import.meta.url), "utf8");
const READER_PITCH_WORKLET = readFileSync(new URL("../assets/reader-pitch-worklet.js", import.meta.url), "utf8");
...
export function createApp({
  ...
  app.get("/", async (req, res) => {
    ...
    const orgCategories = Object.fromEntries(sections.map(({ org }) => [
      org,
      typeof orgs.categoriesFor === "function" ? orgs.categoriesFor(org) : []
    ]));
  ...
  function shellViewerFor(viewer) {
    const shellViewer = { email: viewer.email, org: viewer.org, isAdmin: viewer.isAdmin };
    const displayName = orgs.viewerDisplayName?.(viewer.email) || viewer.displayName;
    if (displayName) shellViewer.displayName = displayName;
    return shellViewer;
  }
  ...
  app.post("/settings/orgs/:name/emails", express.json({ limit: limits.keyJson || "64kb" }), async (req, res) => {
    ...
    if (!viewer) return;
    try {
      const result = orgs.addEmailMember(req.params.name, req.body?.email, req.body?.display_name);
  ...
  app.get("/raw/:id/*", async (req, res) => {
    ...
    let content = req.query.anchor === "1" && req.query.download === undefined && isHtmlContentType(file.contentType)
      ...
      : file.content;
    if (req.query.reader === "1" && req.query.download === undefined && isHtmlContentType(file.contentType)) content = injectReaderBridge(content);
  ...
  app.get("/raw/:id", async (req, res) => {
    ...
    let content = req.query.anchor === "1" && req.query.download === undefined
      ...
      : found.html;
    if (req.query.reader === "1" && req.query.download === undefined) content = injectReaderBridge(content);
  ...
  app.get("/:id", async (req, res) => {
    ...
    return res.set("content-type", "text/html; charset=utf-8").set("cache-control", "no-store")
      .send(pages.shell(meta, nav, reactions.get(viewer.email, id), shellFeedback, { counts, viewers }, shellViewerFor(viewer), orgs.colorMap()[meta.org]));
  });
  ...
  // Public, version-pinned AudioWorklet assets. These contain no artifact or account data and
  // are loaded by the trusted viewer shell, outside the sandboxed artifact frame.
  app.get("/reader-audio/soundtouch-2.1.1.js", (_req, res) => res
    .set({ "content-type": "application/javascript; charset=utf-8", "cache-control": "public, max-age=3600", "x-content-type-options": "nosniff" })
    .send(SOUNDTOUCH_PROCESSOR));
  app.get(["/reader-audio/soundtouch-2.1.1.js.map", "/reader-audio/soundtouch-processor.js.map"], (_req, res) => res
    .set({ "content-type": "application/json; charset=utf-8", "cache-control": "public, max-age=3600", "x-content-type-options": "nosniff" })
    .send(SOUNDTOUCH_SOURCE_MAP));
  app.get("/reader-audio/LICENSE", (_req, res) => res
    .set({ "content-type": "text/plain; charset=utf-8", "cache-control": "public, max-age=3600", "x-content-type-options": "nosniff" })
    .send(SOUNDTOUCH_LICENSE));
  app.get("/reader-audio/pitch-v1.js", (_req, res) => res
    .set({ "content-type": "application/javascript; charset=utf-8", "cache-control": "no-cache", "x-content-type-options": "nosniff" })
  ...
  app.get("/:id/speech/voices", async (req, res) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    const enabled = Boolean(speech && speechEnabledFor(allowed.id));
    return res.set("cache-control", "no-store").json({ enabled, voices: enabled ? speech.voices : [], maxChars: SPEECH_MAX_CHARS });
  });

  app.post("/:id/speech", express.json({ limit: "8kb", strict: true }), async (req, res) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!speech || !speechEnabledFor(allowed.id)) return res.status(503).json({ error: "speech unavailable" });
    const body = req.body;
    if (!body || typeof body !== "object" || Array.isArray(body)) return res.status(400).json({ error: "bad_body" });
    const unknown = Object.keys(body).find((key) => !["text", "voice", "instructions"].includes(key));
    if (unknown) return res.status(400).json({ error: "bad_body" });
    const result = await speech.synthesize(body.text, body.voice, body.instructions);
    if (result.status !== 200) return res.status(result.status).json({ error: result.error });
    return res.set({ "content-type": "audio/wav", "cache-control": "private, no-store" }).send(result.audio);
  });

  app.post("/:id/speech/stream", express.json({ limit: "8kb", strict: true }), async (req, res) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!speech || !speechEnabledFor(allowed.id) || typeof speech.streamSynthesize !== "function") return res.status(503).json({ error: "speech unavailable" });
    const body = req.body;
    if (!body || typeof body !== "object" || Array.isArray(body)) return res.status(400).json({ error: "bad_body" });
    const unknown = Object.keys(body).find((key) => !["text", "voice", "instructions"].includes(key));
    if (unknown) return res.status(400).json({ error: "bad_body" });
    ...
    const result = await speech.streamSynthesize(body.text, body.voice, controller.signal, body.instructions);
    if (result.status !== 200) return res.status(result.status).json({ error: result.error });
    res.status(200).set({ "content-type": "application/vnd.artifact.pcm", "cache-control": "private, no-store", "x-accel-buffering": "no", "x-content-type-options": "nosniff" });
    const reader = result.stream.getReader();
    const waitForDrain = () => new Promise((resolve, reject) => {
      const cleanup = () => { res.off("drain", onDrain); res.off("close", onClose); req.off("aborted", onClose); controller.signal.removeEventListener("abort", onAbort); };
      const onDrain = () => { cleanup(); resolve(); };
      const onClose = () => { cleanup(); reject(new Error("client disconnected")); };
      const onAbort = () => { cleanup(); reject(new Error("request aborted")); };
      res.once("drain", onDrain); res.once("close", onClose); req.once("aborted", onClose); controller.signal.addEventListener("abort", onAbort, { once: true });
    });
    try {
      while (true) {
      ...
      if (!res.headersSent) res.status(503).json({ error: "speech unavailable" }); else res.destroy();
    } finally {
  ...
  app.post("/:id/speech/stream-timed", express.json({ limit: "8kb", strict: true }), async (req, res) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!speech || !speechEnabledFor(allowed.id) || typeof speech.streamTimedSynthesize !== "function") return res.status(503).json({ error: "speech unavailable" });
    const body = req.body;
    if (!body || typeof body !== "object" || Array.isArray(body)) return res.status(400).json({ error: "bad_body" });
    const unknown = Object.keys(body).find((key) => !["text", "voice", "instructions"].includes(key));
    if (unknown) return res.status(400).json({ error: "bad_body" });
    ...
    req.on("aborted", abortRequest);
    res.on("close", abortRequest);
    const result = await speech.streamTimedSynthesize(body.text, body.voice, controller.signal, body.instructions);
    if (result.status !== 200) return res.status(result.status).json({ error: result.error });
    res.status(200).set({ "content-type": "application/vnd.artifact.pcm-timed", "cache-control": "private, no-store", "x-accel-buffering": "no", "x-content-type-options": "nosniff" });
    const reader = result.stream.getReader();
    const waitForDrain = () => new Promise((resolve, reject) => {
      const cleanup = () => { res.off("drain", onDrain); res.off("close", onClose); req.off("aborted", onClose); controller.signal.removeEventListener("abort", onAbort); };
      const onDrain = () => { cleanup(); resolve(); };
  ...
  function stateStoreOrUnavailable(res) {
    if (!state || typeof state.list !== "function") {
  ...
  function validStateKey(key) {
    return typeof key === "string" && key.length > 0 && key.length <= STATE_MAX_KEY_LENGTH && !/[^A-Za-z0-9._-]/.test(key);
  }
  function allowStateRequest(req, viewer) {
    const limit = Number(limits.statePerWindow || STATE_PER_WINDOW);
    const now = Date.now();
    const identity = req.ip || req.socket?.remoteAddress || viewer.email || "unknown";
    const current = stateRate.get(identity);
    const windowMs = Number(limits.stateRateWindowMs || STATE_RATE_WINDOW_MS);
    if (!current || now - current.started >= windowMs) {
      if (stateRate.size >= 10_000) {
        for (const [key, entry] of stateRate) if (now - entry.started >= windowMs) stateRate.delete(key);
        if (stateRate.size >= 10_000) stateRate.delete(stateRate.keys().next().value);
      }
      stateRate.set(identity, { started: now, count: 1 });
      return true;
    }
    current.count += 1;
    return current.count <= limit;
  }
  function stateBody(body) {
    if (!body || typeof body !== "object" || Array.isArray(body) || !Object.hasOwn(body, "value")) throw new Error("bad_body");
    const fields = Object.keys(body);
    if (fields.some((field) => field !== "value" && field !== "if_revision")) throw new Error("bad_body");
    if (Object.hasOwn(body, "if_revision") && (!Number.isSafeInteger(body.if_revision) || body.if_revision < 0)) throw new Error("bad_revision");
    return { value: body.value, ifRevision: body.if_revision === undefined ? null : body.if_revision };
  }
  function stateNamespace(req, viewer) {
    const raw = req.query?.scope;
    if (Array.isArray(raw) || (raw !== undefined && typeof raw !== "string")) return null;
    const scope = raw === undefined ? "org" : raw;
    if (scope !== "org" && scope !== "viewer") return null;
    return { scope, viewer: scope === "viewer" ? String(viewer.email || "").trim().toLowerCase() : "" };
  }
  app.get("/:id/state", async (req, res) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!stateStoreOrUnavailable(res)) return;
    if (!allowStateRequest(req, allowed.viewer)) return res.status(429).json({ error: "rate_limited" });
    const namespace = stateNamespace(req, allowed.viewer);
    if (!namespace) return res.status(400).json({ error: "bad_scope" });
    if (namespace.scope === "viewer" && !namespace.viewer) return res.status(404).json({ error: "Not found" });
    try { return res.set("cache-control", "no-store").json({ keys: state.list(allowed.id, namespace.scope, namespace.viewer) }); }
    catch (error) { logger.error?.("[artifact-mcp] state list failed", error); return res.status(500).json({ error: "state_unavailable" }); }
  });
  app.get("/:id/state/:key", async (req, res) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!stateStoreOrUnavailable(res)) return;
    if (!allowStateRequest(req, allowed.viewer)) return res.status(429).json({ error: "rate_limited" });
    ...
    if (!namespace) return res.status(400).json({ error: "bad_scope" });
    if (namespace.scope === "viewer" && !namespace.viewer) return res.status(404).json({ error: "Not found" });
    if (!validStateKey(req.params.key)) return res.status(400).json({ error: "bad_key" });
    let found;
    try { found = state.get(allowed.id, req.params.key, namespace.scope, namespace.viewer); }
    catch (error) { logger.error?.("[artifact-mcp] state read failed", error); return res.status(500).json({ error: "state_unavailable" }); }
    if (!found) return res.status(404).json({ error: "Not found" });
    const response = namespace.scope === "viewer" ? { ...found } : found;
    if (namespace.scope === "viewer") delete response.updated_by;
    return res.set("cache-control", "no-store").json(response);
  });
  const stateJsonParser = express.json({ limit: limits.stateJson || STATE_JSON_LIMIT, strict: false });
  app.put("/:id/state/:key", async (req, res, next) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!stateNamespace(req, allowed.viewer)) return res.status(400).json({ error: "bad_scope" });
    req.viewerStateGrant = allowed;
    stateJsonParser(req, res, (error) => {
      if (error) return res.status(error.status === 413 ? 413 : 400).json({ error: error.status === 413 ? "too_large" : "bad_body" });
      next();
    });
  }, async (req, res) => {
    const allowed = req.viewerStateGrant || await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!stateStoreOrUnavailable(res)) return;
    if (!allowStateRequest(req, allowed.viewer)) return res.status(429).json({ error: "rate_limited" });
    const namespace = stateNamespace(req, allowed.viewer);
    ...
    if (!validStateKey(req.params.key)) return res.status(400).json({ error: "bad_key" });
    let input;
    try { input = stateBody(req.body); } catch (error) { return res.status(400).json({ error: String(error.message || error) }); }
    let result;
    try { result = state.put(allowed.id, req.params.key, input.value, allowed.viewer.email, input.ifRevision, namespace.scope, namespace.viewer); }
    catch (error) { logger.error?.("[artifact-mcp] state write failed", error); return res.status(500).json({ error: "state_unavailable" }); }
    if (result.ok) return res.json({ key: result.key, revision: result.revision, updated_at: result.updated_at });
    if (result.reason === "too_large") return res.status(413).json({ error: "too_large" });
    if (result.reason === "too_many_keys") return res.status(409).json({ error: "too_many_keys" });
    if (result.reason === "conflict") return res.status(409).json({ error: "conflict", value: result.value, revision: result.revision });
    return res.status(400).json({ error: result.reason });
  });
  app.delete("/:id/state/:key", async (req, res) => {
    const allowed = await artifactApiOr404(req, res);
    if (!allowed) return;
    if (!stateStoreOrUnavailable(res)) return;
    if (!allowStateRequest(req, allowed.viewer)) return res.status(429).json({ error: "rate_limited" });
    ...
    if (!validStateKey(req.params.key)) return res.status(400).json({ error: "bad_key" });
    try {
      state.remove(allowed.id, req.params.key, () => {
        audit?.append(auditFor(req, allowed.viewer, allowed.meta.org), { operation: "state.delete", targetType: "artifact_state", targetId: allowed.id, result: "success", classification: "viewer_state" });
      }, namespace.scope, namespace.viewer);
    }
    catch (error) { logger.error?.("[artifact-mcp] state delete failed", error); return res.status(500).json({ error: "state_unavailable" }); }
