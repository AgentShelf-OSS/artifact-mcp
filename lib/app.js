...
export function createApp({
  ...
  app.get("/:id/state", async (req, res) => {
    ...
    try { return res.set("cache-control", "no-store").json({ keys: state.list(allowed.id) }); }
  ...
  app.get("/:id/state/:key", async (req, res) => {
    ...
    let found;
    try { found = state.get(allowed.id, req.params.key); }
    ...
    return res.set("cache-control", "no-store").json(found);
  });
  ...
  app.put("/:id/state/:key", async (req, res, next) => {
    ...
    let result;
    try { result = state.put(allowed.id, req.params.key, input.value, allowed.viewer.email, input.ifRevision); }
