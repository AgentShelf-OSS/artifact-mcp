...
function containsRequestMeta(payload) {
  if (Array.isArray(payload)) return payload.some(containsRequestMeta);
  return Boolean(payload?.params && Object.hasOwn(payload.params, "_meta"));
}
...
export function validateMcpHttpRequest(payload, headers = {}) {
  ...
  const method = payload && typeof payload === "object" && !Array.isArray(payload)
    ? payload.method
    : undefined;
  const modernIntent = (versionHeader && versionHeader !== PROTOCOL_VERSION)
    || containsRequestMeta(payload)
