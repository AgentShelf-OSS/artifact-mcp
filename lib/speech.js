...
export function createSpeechService({ fetchImpl = globalThis.fetch, env = process.env } = {}) {
  ...
  const addClient = (prefix, rawValue, tokenValue) => {
    ...
    if (token) clients.set(prefix, { prefix, endpoint: new URL("speech", url).toString(), streamEndpoint: new URL("speech/stream", url).toString(), token, busy: false });
    } catch {}
  };
  ...
  return {
    ...
    async synthesize(text, voice, instructions) {
      ...
      const prefix = voiceId.startsWith("pocket_") ? "pocket" : voiceId.startsWith("qwen_") ? "qwen" : voiceId.startsWith("moss_") ? "moss" : "kokoro";
      const client = clients.get(prefix);
      const validVoices = prefix === "pocket" ? POCKET_SPEECH_VOICES : prefix === "qwen" ? QWEN_SPEECH_VOICES : prefix === "moss" ? MOSS_SPEECH_VOICES : SPEECH_VOICES;
    ...
    async streamSynthesize(text, voice, signal, instructions) {
      const voiceId = String(voice || "");
      const prefix = voiceId.startsWith("qwen_") ? "qwen" : voiceId.startsWith("pocket_") ? "pocket" : "";
      const voiceList = prefix === "qwen" ? QWEN_SPEECH_VOICES : prefix === "pocket" ? POCKET_SPEECH_VOICES : [];
      const client = clients.get(prefix);
