...
// Target ids are server-generated identifiers (artifact ids may begin with a digit); same alphabet as the Rust ledger.
const SAFE_TARGET_ID = /^[a-z0-9][a-z0-9._-]{0,99}$/;
...
function normalizeEvent(context, event) {
  ...
  if ((targetId && webhookTarget && !isValidAuditWebhookId(targetId))
      || (targetId && !webhookTarget && !SAFE_TARGET_ID.test(targetId))
