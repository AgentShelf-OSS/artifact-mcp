...
export const READER_BRIDGE_MARKER = "artifact-reader-bridge";
export const READER_BRIDGE = readFileSync(new URL("../assets/reader-ereader.js", import.meta.url), "utf8") + "\n" + readFileSync(new URL("../assets/reader-bridge.js", import.meta.url), "utf8");
...
export function injectReaderBridge(content) {
  const html = Buffer.isBuffer(content) ? content.toString("utf8") : String(content);
  const bridge = `<script id="${READER_BRIDGE_MARKER}">${READER_BRIDGE}</script>`;
  const re = /<\/body\s*>/gi;
  let last = -1;
  for (let m = re.exec(html); m; m = re.exec(html)) last = m.index;
  return last === -1 ? `${html}${bridge}` : `${html.slice(0, last)}${bridge}${html.slice(last)}`;
}
