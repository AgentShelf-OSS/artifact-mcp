...
function preserveAnchorNavigation(ev){try{var link=ev.target&&ev.target.closest&&ev.target.closest("a[href]");if(!link||link.hasAttribute("download"))return;var url=new URL(link.href,w.location.href),root=w.location.pathname.match(/^(.*\\/raw\\/[^/]+\\/)/),bundlePrefix=root&&new URL(root[1],w.location.href).href,inBundle=!!bundlePrefix&&url.href.startsWith(bundlePrefix),outbound=(url.protocol==="http:"||url.protocol==="https:")&&!inBundle;if(outbound){ev.preventDefault();post({type:"anchor:navigate",href:url.href});return;}if(link.target&&link.target!=="_self")return;if(!inBundle)return;url.searchParams.set("anchor","1");link.href=url.href;}catch(_){}}
