...
const MIGRATIONS = [
  ...
  },
  {
    ...
    up(db) {
      // PBI-084: the anchor-v2 dense metadata; NULL retains existing anchor semantics.
      ensureColumn(db, "feedback", "anchor_kind", "TEXT");
      ensureColumn(db, "feedback", "anchor_node_id", "TEXT");
      ensureColumn(db, "feedback", "anchor_quote", "TEXT");
    }
  },
  {
    version: 33,
  ...
  },
  {
    ...
    up(db) {
      // Carry the v33 organization rows into the explicit org bucket before adding
      // per-viewer namespaces.  The rebuild keeps this compatible with SQLite's
      // primary-key limitations around ALTER TABLE.
      db.exec(`
        ...
        INSERT INTO artifact_state_v2
          (artifact_id, scope, viewer, key, value, revision, updated_at, updated_by)
          SELECT artifact_id, 'org', '', key, value, revision, updated_at, updated_by
          FROM artifact_state;
        DROP TABLE artifact_state;
        ALTER TABLE artifact_state_v2 RENAME TO artifact_state;
      `);
      ensureColumn(db, "org_email_members", "display_name", "TEXT NOT NULL DEFAULT ''");
    }
  }
];
