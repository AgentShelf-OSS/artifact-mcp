...
function invoke(app, method, path, { params = {}, body, headers = {} } = {}) {
  ...
  return new Promise((resolve, reject) => {
    res.end = function end(chunk) { const text = chunk == null ? "" : chunk.toString(); let value; try { value = JSON.parse(text); } catch { value = text || undefined; } resolve({ status: res.statusCode, body: value }); socket.destroy(); return res; };
...
function fixture(viewer = { email: "viewer@acme.test", org: "acme", isAdmin: false }) {
  ...
  return createApp({
    ...
    speech: { voices: [{ id: "af_heart", name: "Heart · American" }], synthesize: async () => ({ status: 200, audio: Buffer.from("RIFF") }) },
    logger: { info() {}, error() {} }
  });
}
