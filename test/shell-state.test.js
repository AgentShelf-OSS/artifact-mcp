...
function harness({ enabled = true, entries = new Map() } = {}) {
  let now = 0, timerId = 0;
  ...
  const broker = createBroker({ enabled, artifactId: "artifact", storage, frame: () => frame,
  ...
  return {
    ...
    cache: key => JSON.parse(entries.get("artifact-state:artifact:" + key) || "null"),
  };
}
...
test("hello strips timestamps and writer identities from bridge metadata", async () => {
  const h = harness(); h.send({ type: "state:hello" });
  await h.reply(0, 200, { keys: [{ key: "note", revision: 2, updated_at: "now", updated_by: "private@example.test" }] });
  assert.deepEqual(h.messages, [{ type: "state:ready", enabled: true, scope: "org", keys: [{ key: "note", revision: 2 }] }]);
});
...
test("sets debounce per key for 500ms and acknowledge only after the server", async () => {
  ...
  assert.deepEqual(h.messages, [{ type: "state:saved", key: "__proto__", revision: 1 }]);
...
test("cached reads refresh changed revisions and emit absence after server deletion", async () => {
  ...
  assert.deepEqual(h.messages[0], { type: "state:value", key: "note", value: "old", revision: 3 });
  await h.reply(0, 200, { value: "fresh", revision: 4, updated_by: "private@example.test" });
  assert.deepEqual(h.messages[1], { type: "state:value", key: "note", value: "fresh", revision: 4 });
  ...
  assert.deepEqual(h.messages.at(-1), { type: "state:value", key: "note", value: null, revision: 0 });
...
test("old reads cannot overwrite a newer acknowledged mutation", async () => {
  ...
  assert.deepEqual(h.messages, [{ type: "state:saved", key: "note", revision: 2 }]);
});
...
test("conflicts refresh cache then return canonical value before the error", async () => {
  ...
  assert.deepEqual(h.messages, [
    { type: "state:value", key: "note", value: "theirs", revision: 2, conflict: true },
    { type: "state:error", key: "note", reason: "conflict" },
  ]);
...
test("invalid JSON, keys, revisions and size fail locally; capacity errors stay in the protocol", async () => {
  ...
  assert.deepEqual(h.messages.at(-1), { type: "state:error", key: "note", reason: "too_large" });
});
